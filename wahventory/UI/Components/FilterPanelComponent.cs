...
public class FilterPanelComponent
{
    ...
    /// Used by the redesigned inventory window's left sidebar.
    /// </summary>
    public void DrawSidebar(InventorySettings settings)
    {
        var filters = settings.SafetyFilters;
        ...
        changed |= SidebarCheckbox("Currency",            () => filters.FilterCurrencyItems,    v => filters.FilterCurrencyItems = v,    "Gil, tomestones, MGP, item IDs < 100");
        changed |= SidebarCheckbox("Crystals & Shards",   () => filters.FilterCrystalsAndShards, v => filters.FilterCrystalsAndShards = v, "Aether crystals, shards, clusters");
        changed |= SidebarCheckbox("In Gearset",          () => filters.FilterGearsetItems,     v => filters.FilterGearsetItems = v,     "Equipment in any saved gearset");
        changed |= SidebarCheckbox("Indisposable",        () => filters.FilterIndisposableItems, v => filters.FilterIndisposableItems = v, "Items the game won't let you discard");
        changed |= SidebarCheckbox("Ultimate / Special",  () => filters.FilterUltimateTokens,   v => filters.FilterUltimateTokens = v,   "Raid tokens, retired tomestones");
        ...
        changed |= SidebarCheckbox("HQ Items",             () => filters.FilterHQItems,           v => filters.FilterHQItems = v,           "High-Quality flagged items");
        changed |= SidebarCheckbox("Collectables",         () => filters.FilterCollectables,      v => filters.FilterCollectables = v,      "Collectability turn-in items");
        changed |= SidebarCheckbox("Unique & Untradeable", () => filters.FilterUniqueUntradeable, v => filters.FilterUniqueUntradeable = v, "Cannot be reacquired or sold");
    ...
    private static bool SidebarCheckbox(string label, Func<bool> get, Action<bool> set, string tooltip)
    {
        var v = get();
